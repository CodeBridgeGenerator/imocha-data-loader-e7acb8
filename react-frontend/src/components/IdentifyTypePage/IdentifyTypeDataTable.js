...
const IdentifyTypeDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.identifytype}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "identifyType"});
...
<Column field="identifytype" header="Identify Type" body={pTemplate0} filter={selectedFilterFields.includes("identifytype")} hidden={selectedHideFields?.includes("identifytype")}  sortable style={{ minWidth: "8rem" }} />
            <Column header="Edit" body={editTemplate} />
            <Column header="Delete" body={deleteTemplate} />
            
        </DataTable>
        <Dialog header="Upload IdentifyType Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search IdentifyType" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default IdentifyTypeDataTable;