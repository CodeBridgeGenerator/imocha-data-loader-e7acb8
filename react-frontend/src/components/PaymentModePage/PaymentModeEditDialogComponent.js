...
const PaymentModeCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        ...
        const result = await client.service("paymentMode").patch(_entity._id, _data);
        props.onHide();
        props.alert({ type: "success", title: "Edit info", message: "Info paymentMode updated successfully" });
    ...
    return (
        <Dialog header="Edit Payment Mode" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="paymentMode-edit-dialog-component">
