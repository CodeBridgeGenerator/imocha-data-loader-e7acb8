...
const PaymentModeDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.paymentmode}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "paymentMode"});
...
<Column field="paymentmode" header="Payment Mode" body={pTemplate0} filter={selectedFilterFields.includes("paymentmode")} hidden={selectedHideFields?.includes("paymentmode")}  sortable style={{ minWidth: "8rem" }} />
            <Column header="Edit" body={editTemplate} />
            <Column header="Delete" body={deleteTemplate} />
            
        </DataTable>
        <Dialog header="Upload PaymentMode Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search PaymentMode" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default PaymentModeDataTable;