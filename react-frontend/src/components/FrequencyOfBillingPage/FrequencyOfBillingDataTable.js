...
const FrequencyOfBillingDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.frequencyofbilling}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "frequencyOfBilling"});
...
<Column field="frequencyofbilling" header="Frequency of Billing" body={pTemplate0} filter={selectedFilterFields.includes("frequencyofbilling")} hidden={selectedHideFields?.includes("frequencyofbilling")}  sortable style={{ minWidth: "8rem" }} />
            <Column header="Edit" body={editTemplate} />
            <Column header="Delete" body={deleteTemplate} />
            
        </DataTable>
        <Dialog header="Upload FrequencyOfBilling Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search FrequencyOfBilling" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default FrequencyOfBillingDataTable;