...
const StateCodeDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.statecode}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "stateCode"});
...
<Column field="statecode" header="State Code" body={pTemplate0} filter={selectedFilterFields.includes("statecode")} hidden={selectedHideFields?.includes("statecode")}  sortable style={{ minWidth: "8rem" }} />
            <Column header="Edit" body={editTemplate} />
            <Column header="Delete" body={deleteTemplate} />
            
        </DataTable>
        <Dialog header="Upload StateCode Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search StateCode" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default StateCodeDataTable;