...
const ClassificationCodeDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.classificationcode}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "classificationCode"});
...
<Column field="classificationcode" header="Classification Code" body={pTemplate0} filter={selectedFilterFields.includes("classificationcode")} hidden={selectedHideFields?.includes("classificationcode")}  sortable style={{ minWidth: "8rem" }} />
            <Column header="Edit" body={editTemplate} />
            <Column header="Delete" body={deleteTemplate} />
            
        </DataTable>
        <Dialog header="Upload ClassificationCode Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search ClassificationCode" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default ClassificationCodeDataTable;