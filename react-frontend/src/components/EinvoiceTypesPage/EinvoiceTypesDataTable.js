...
const EinvoiceTypesDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.einvoicetypes}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "einvoiceTypes"});
...
<Column field="einvoicetypes" header="eInvoice Types" body={pTemplate0} filter={selectedFilterFields.includes("einvoicetypes")} hidden={selectedHideFields?.includes("einvoicetypes")}  sortable style={{ minWidth: "8rem" }} />
            <Column header="Edit" body={editTemplate} />
            <Column header="Delete" body={deleteTemplate} />
            
        </DataTable>
        <Dialog header="Upload EinvoiceTypes Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search EinvoiceTypes" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default EinvoiceTypesDataTable;