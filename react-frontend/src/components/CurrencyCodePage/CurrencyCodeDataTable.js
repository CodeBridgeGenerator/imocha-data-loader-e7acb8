...
const CurrencyCodeDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.currencycode}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "currencyCode"});
...
<Column field="currencycode" header="Currency Code" body={pTemplate0} filter={selectedFilterFields.includes("currencycode")} hidden={selectedHideFields?.includes("currencycode")}  sortable style={{ minWidth: "8rem" }} />
            <Column header="Edit" body={editTemplate} />
            <Column header="Delete" body={deleteTemplate} />
            
        </DataTable>
        <Dialog header="Upload CurrencyCode Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search CurrencyCode" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default CurrencyCodeDataTable;