...
const CountryCodeDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.countrycode}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "countryCode"});
...
<Column field="countrycode" header="Country Code" body={pTemplate0} filter={selectedFilterFields.includes("countrycode")} hidden={selectedHideFields?.includes("countrycode")}  sortable style={{ minWidth: "8rem" }} />
            <Column header="Edit" body={editTemplate} />
            <Column header="Delete" body={deleteTemplate} />
            
        </DataTable>
        <Dialog header="Upload CountryCode Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search CountryCode" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default CountryCodeDataTable;