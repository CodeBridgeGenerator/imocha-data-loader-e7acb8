...
const PhoneNumberPrefixDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.phonenumberprefix}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "phoneNumberPrefix"});
...
<Column field="phonenumberprefix" header="Phone Number Prefix" body={pTemplate0} filter={selectedFilterFields.includes("phonenumberprefix")} hidden={selectedHideFields?.includes("phonenumberprefix")}  sortable style={{ minWidth: "8rem" }} />
            <Column header="Edit" body={editTemplate} />
            <Column header="Delete" body={deleteTemplate} />
            
        </DataTable>
        <Dialog header="Upload PhoneNumberPrefix Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search PhoneNumberPrefix" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default PhoneNumberPrefixDataTable;