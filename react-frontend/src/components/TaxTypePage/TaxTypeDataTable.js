...
const TaxTypeDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.taxtype}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "taxType"});
...
<Column field="taxtype" header="Tax Type" body={pTemplate0} filter={selectedFilterFields.includes("taxtype")} hidden={selectedHideFields?.includes("taxtype")}  sortable style={{ minWidth: "8rem" }} />
            <Column header="Edit" body={editTemplate} />
            <Column header="Delete" body={deleteTemplate} />
            
        </DataTable>
        <Dialog header="Upload TaxType Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
      ...
      <Dialog header="Search TaxType" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default TaxTypeDataTable;